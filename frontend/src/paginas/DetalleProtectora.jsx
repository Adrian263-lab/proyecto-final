  ...
  return (
    <div className="container mt-4 animate__animated animate__fadeIn">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <button onClick={compartirPerfil} className="btn btn-sm btn-outline-secondary rounded-pill px-3 shadow-sm">
            <i className="bi bi-share me-2"></i>Compartir perfil
        </button>
      </div>
      
      {/* CABECERA CON LOGO DINÁMICO */}
      <div className="card border-0 shadow-sm p-4 my-4 rounded-4 bg-white border-top border-huellitas border-5">
        <div className="row align-items-center">
            <div className="col-auto">
                {/* Muestra el logo si existe, si no, el icono de la casita */}
                {protectora.logo_url ? (
                  <img 
                    src={protectora.logo_url} 
                    alt={protectora.name} 
                    className="rounded-circle shadow border border-3 border-light"
                    style={{ width: '100px', height: '100px', objectFit: 'cover', backgroundColor: '#f8f9fa' }}
                  />
                ) : (
                  <div className="bg-huellitas text-white rounded-circle d-flex align-items-center justify-content-center shadow" style={{width: '90px', height: '90px'}}>
            ...
            <div className="col mt-3 mt-md-0">
                ...
                <p className="text-muted mb-0 small">
                    <i className="bi bi-geo-alt-fill me-1 text-warning"></i>{protectora.direccion || 'Ubicación no disponible'} 
                    <span className="mx-2 text-light">|</span>
                    <i className="bi bi-envelope-fill me-1 text-warning"></i>{protectora.email}
                </p>
            </div>
        </div>
      </div>
      ...
      <div className="row align-items-center mb-4 border-bottom g-0">
        <div className="col-md-6 d-flex">
            <button onClick={() => { setPestana('adopcion'); setBusqueda(''); }}
                className={`btn btn-lg px-4 py-3 border-0 rounded-0 ${pestana === 'adopcion' ? 'text-huellitas border-bottom border-huellitas border-3 fw-bold' : 'text-muted'}`}>
                🐾 Adopción ({enAdopcion.length})
            </button>
            <button onClick={() => { setPestana('historial'); setBusqueda(''); }}
        ...
        {pestana === 'adopcion' ? (
          enAdopcion.length > 0 ? (
            enAdopcion.map(animal => (
              <div className="col-md-4 col-lg-3" key={animal.id}>
                <Link to={`/animal/${animal.id}`} className="text-decoration-none d-block shadow-sm rounded-4 h-100 card-link-container">
                  <div className="card h-100 border-0 rounded-4 text-center p-3" style={{ pointerEvents: 'none' }}>
                    <div className="mb-3 mx-auto overflow-hidden rounded-circle border border-3 border-white shadow-sm" style={{ width: '120px', height: '120px' }}>
                      <img 
                    ...
                    <span className="badge bg-huellitas-light text-huellitas rounded-pill py-2 w-100">
                      Ver ficha
                    </span>
                  </div>
                </Link>
          ...
          historialAdoptados.length > 0 ? (
            historialAdoptados.map(animal => (
              <div className="col-md-4 col-lg-3" key={animal.id}>
                <div className="card h-100 border-0 shadow-sm rounded-4 text-center p-3 bg-light opacity-75">
                  ...
                  <div className="badge bg-success rounded-pill small">¡Adoptado!</div>
                </div>
              </div>
            ))
      ...
      <style>{`
        ...
        .bg-huellitas-light { background-color: #f1f0ff; }
