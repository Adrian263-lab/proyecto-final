...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Crear 2 animales y 5 eventos para Huellitas
        for ($i = 1; $i <= 2; $i++) {
            Animal::create([
                'nombre' => 'Mascota Huellitas ' . $i, 
                'especie_id' => $perro->id, 
                'user_id' => $protectora1->id, 
                'raza' => 'Común', 
                'estado' => 'En adopción', 
                'descripcion' => 'Descripción fija',
                'sexo' => 'Macho'
            ]);
        }
        for ($i = 1; $i <= 5; $i++) {
        ...
        foreach ($protectorasExtra as $data) {
            ...
            for ($i = 1; $i <= 3; $i++) {
                Animal::create([
                    'nombre' => 'Mascota de ' . $user->name . ' ' . $i,
                    'especie_id' => $perro->id,
                    'user_id' => $user->id, // <- Corregido: usa el ID del user del bucle
                    'raza' => 'Común',
                    'estado' => 'En adopción',
                    'descripcion' => 'Descripción fija',
                    'sexo' => 'Hembra'
